...
describe('Vanguard impostor frame assets', () => {
  ...
  it('keeps canonical heading math and specialized frame remap unchanged', () => {
    expect(VANGUARD_IMPOSTOR_FRAME_REMAP).toEqual([0, 1, 2, 3, 4, 5, 6, 7]);
    expect(VANGUARD_IMPOSTOR_HEADING_OFFSET_DEGREES).toBe(0);
  });

  it('builds base-aware cache-busted URLs and wraps calibrated runtime indices', () => {
    ...
    expect(vanguardImpostorFrameUrl(7, '/pepepow-elemental-front/'))
      .toBe(`/pepepow-elemental-front/assets/impostors/vanguard/01-front-left.webp${revision}`);
