...
- v18 stabilizes melee contact under crowd pressure. Same-faction melee units already attacking the same target fan tangentially around a deterministic combat ring rather than repeatedly pushing one another outside attack range, while established hostile melee contact keeps the defender/heavier body anchored so a surrounded target is not shoved across the battlefield.
...
- v24 gives strategic buildings physical navigation footprints. The blocked core is intentionally smaller than the full rendered silhouette: Core/Barracks/Workshop block a 3×3 square, Arcane Tower/Outpost a 3×3 cross, and Extractor/Mana Well only their center cell. Construction blocks immediately; destroyed resource buildings release the space. Trained units spawn outside the producer footprint. Friendly-unit phasing remains unchanged.
